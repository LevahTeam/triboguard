...
![One real MCF7 image in three panels: the cells as imaged, half of them outlined as damaged, and Cellpose's result with found cells in green and missed cells in red](media/1_blind_spot_zoom.png)

One real MCF7 image from the pre-registered test. Half its cells were damaged by
coin flip, and Cellpose found 146 of the 158 untouched cells and 1 of the 151
damaged ones. The frame is drawn by re-running the sweep's exact recipe on that
image, and the script refuses to draw it unless the re-run matches the recorded
result.

![Detection rate against damage severity on four cell lines: healthy and damaged cells are found equally at zero damage, then damaged-cell detection falls to near zero](media/3_damage_vs_detection.png)

The same gap on every line, over 20 real images per point. At zero damage the two
groups are found equally, which is the built-in control. All three figures,
including [the whole-image view](media/2_blind_spot_whole_image.png), are
regenerated by `python scripts/make_video_figures.py`.
