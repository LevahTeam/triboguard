...
def build_synthetic_experiment(
    ...
    *,
    ...
    fields: int = 2,
    effect: bool = True,
    timepoints: tuple[float, ...] = (24.0,),
    longitudinal: bool = True,
    half_time_hours: float = 12.0,
    seed: int = 0,
) -> Path:
    ...
    With more than one entry in ``timepoints`` the effect develops over time with
    the given half-time, so kinetics can be recovered. ``longitudinal`` reuses the
    same well across timepoints, as non-destructive imaging allows; setting it
    False gives a fresh well per timepoint, as a destructive assay would force.
    """
    import csv as _csv
        ...
        for concentration in concentrations:
            for replicate in range(wells_per_condition):
                ...
                for exposure in timepoints:
                    ...
                    progress = (
                        1.0 - math.exp(-math.log(2) * exposure / half_time_hours)
                        if len(timepoints) > 1
                        else 1.0
                    )
                    if longitudinal:
                    ...
                    viability = float(
                        np.clip(1.0 - 0.85 * strength * progress + rng.normal(0, 0.03), 0.0, 1.2)
                    )
                    for field in range(1, fields + 1):
                        ...
                        for cy, cx in ((28, 28), (28, 68), (68, 28), (68, 68)):
                            ...
                            inside = ((ys - cy) ** 2 + (xs - cx) ** 2) <= (radius + jitter) ** 2
                            canvas[inside] = 225
                        name = f"{day}_{well}_t{exposure:g}_f{field}.png"
